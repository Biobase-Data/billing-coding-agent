...
| case_04 | 4-antibody IHC melanoma panel, all antibodies named -> 88342 (first) + 88341 x3 (additional); diagnosis is site-coded from the specimen (right upper back -> trunk -> C43.59), which is on the covered-diagnosis list (a malignant finding is at least as clear a medical-necessity justification as a benign one), so no coverage finding fires |
| case_05 | 2-antibody IHC panel, one antibody named, one unnamed -> `documentation:ihc_missing_antibody` review finding, no coverage finding (left calf -> lower limb -> C43.72, also covered) |
...
| case_15_initial / case_15_amended | same case_id (`S26-0042900`), same date_of_service -- an amended report changes the diagnosis from compound nevus (site: upper back -> trunk -> D22.5) to a qualified atypical melanocytic proliferation (D48.5). `pipeline/cli/diff.py` diffs the two runs: the level code (88305) and rule version are unchanged; the diagnosis line and two findings (`coverage:DEMO-POLICY-88305`, `diagnosis:qualified_certainty`) are new. See `pipeline/eval/test_diff.py`. |
